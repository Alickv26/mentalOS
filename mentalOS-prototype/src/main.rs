...
use mentalOS::router::{CommandExecutor, CommandRouter, FirejailExecutor};
...
fn main() {
    ...
    thread::spawn(move || {
        ...
        rt.block_on(async {
            ...
            let memory = Arc::new(Mutex::new(MemoryManager::new(workspace_dir)));
            let executor = FirejailExecutor;

            let mut router = CommandRouter::new(openclaw, whitelist, memory, executor);

            while let Some(req) = backend_rx.recv().await {
                match req {
                    BackendRequest::Input { text, workspace, category } => {
